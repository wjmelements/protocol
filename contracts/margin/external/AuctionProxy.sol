...
contract AuctionProxy
{
        ...
        returns (uint256)
    {
        ...
        if (!margin.containsPosition(positionId)) {
            return 0; // if position is closed, consider this a success and return zero
        }
        ...
        if (maxCloseAmount == 0) {
            return 0; // if order cannot be used, consider this a success and return zero
        }

        if (maxCloseAmount < minCloseAmount) {
            return 0; // if order is already taken, consider this a success and return zero
        }
